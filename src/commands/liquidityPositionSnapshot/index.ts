...
export default class liquidityPositionSnapshot extends Command {
  ...
  async run() {
    const { flags } = this.parse(liquidityPositionSnapshot);
    await handler({ command: this, query: LiquidityPositionSnapshotDocument, variables: flags });
  }
}
