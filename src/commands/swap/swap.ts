...
export default class swap extends Command {
  ...
  async run() {
    const { flags } = this.parse(swap);
    await handler({ command: this, query: SwapDocument, variables: flags });
  }
}
