...
export default class liquidityPositionSnapshots extends Command {
  ...
  async run() {
    const { flags } = this.parse(liquidityPositionSnapshots);
    await handler({ command: this, query: LiquidityPositionSnapshotsDocument, variables: flags });
  }
}
