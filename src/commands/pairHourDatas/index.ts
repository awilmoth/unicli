...
export default class pairHourDatas extends Command {
  ...
  async run() {
    const { flags } = this.parse(pairHourDatas);
    await handler({ command: this, query: PairHourDatasDocument, variables: flags });
  }
}
