...
export default class tokenDayData extends Command {
  ...
  async run() {
    const { flags } = this.parse(tokenDayData);
    await handler({ command: this, query: TokenDayDataDocument, variables: flags });
  }
}
