...
export default class mints extends Command {
  ...
  async run() {
    const { flags } = this.parse(mints);
    await handler({ command: this, query: MintsDocument, variables: flags });
  }
}
