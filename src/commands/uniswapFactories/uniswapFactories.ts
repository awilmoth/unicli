...
const UniswapFactoriesDocument = `
...
export default class uniswapFactories extends Command {
  ...
  async run() {
    const { flags } = this.parse(uniswapFactories);
    await handler({ command: this, query: UniswapFactoriesDocument, variables: flags });
  }
}
