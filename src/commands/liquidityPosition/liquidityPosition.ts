...
export default class liquidityPosition extends Command {
  ...
  async run() {
    const { flags } = this.parse(liquidityPosition);
    await handler({ command: this, query: LiquidityPositionDocument, variables: flags });
  }
}
