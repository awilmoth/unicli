...
export default class uniswapDayData extends Command {
  ...
  async run() {
    const { flags } = this.parse(uniswapDayData);
    await handler({ command: this, query: UniswapDayDataDocument, variables: flags });
  }
}
