...
export default class tokens extends Command {
  ...
  async run() {
    const { flags } = this.parse(tokens);
    await handler({ command: this, query: TokensDocument, variables: flags });
  }
}
